...
FieldValue DecodeFieldValueImpl(Reader* reader) {
  ...
  if (reader->bytes_left() == 0) {
    reader->set_status(
        Status(FirestoreErrorCode::DataLoss, "Input Value proto missing contents"));
  ...
  while (reader->bytes_left()) {
    ...
    switch (tag.field_number) {
      ...
      default:
        ...
        FIREBASE_ASSERT_MESSAGE(
            false,
            "Unhandled message field number (tag): %i.",
            tag.field_number);
        reader->set_status(Status(
            FirestoreErrorCode::DataLoss,
    ...
    switch (tag.field_number) {
      ...
      case google_firestore_v1beta1_Value_string_value_tag:
        ...
        break;
      case google_firestore_v1beta1_Value_timestamp_value_tag:
        // TODO(rsgowman): We should merge the existing timestamp (if any) with
        // the newly parsed timestamp.
