...
TEST_F(SerializerTest, EncodesFieldValuesWithRepeatedEntries) {
  ...
  // won't generate that, so to test, we either need to use hand-crafted, raw bytes
  // or use a proto message that's *almost* the same as the real one, such that
  // when it's encoded, you can generate these repeated fields. (This is how
  // libprotobuf tests itself.)
  //
  ...
  const pb_field_t google_firestore_v1beta1_Value_fields_Fake[2] = {
    PB_FIELD(1, BOOL, SINGULAR, STATIC, FIRST, google_firestore_v1beta1_Value_Fake, boolean_value, boolean_value, 0),
    PB_FIELD(2, INT64   , SINGULAR, STATIC  , OTHER, google_firestore_v1beta1_Value_Fake, integer_value, boolean_value, 0),
  };
